...
describe('accounts.utils', () => {
  ...
  describe('toVaultBalance', () => {
    it.each([
      ...
      const cachedSnapshot = JSON.parse(JSON.stringify(MOCK_VAULT_SNAPSHOT));
      cachedSnapshot.balance = snapshot.balance;
      cachedSnapshot.pricePerFullShare = snapshot.balance / snapshot.totalSupply;
      jest.spyOn(vaultsUtils, 'getCachedVault').mockImplementation(async (_c, _v) => cachedSnapshot);
