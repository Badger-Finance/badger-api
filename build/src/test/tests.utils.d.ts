...
export declare const TEST_CHAIN: Arbitrum | BinanceSmartChain | Ethereum | Fantom | import("../chains/config/optimism.config").Optimism | Polygon;
export declare const CURRENT_BLOCK = 0;
export declare function mockVaultDTO(address: string): VaultDTO;
export declare function setupMapper(items: unknown[], filter?: (items: unknown[]) => unknown[]): jest.SpyInstance<QueryIterator<StringToAnyObjectMap>, [parameters: import("@aws/dynamodb-data-mapper").QueryParameters<StringToAnyObjectMap>]>;
export declare function setupBatchGet(items: unknown[], filter?: (items: unknown[]) => unknown[]): jest.SpyInstance<AsyncIterableIterator<StringToAnyObjectMap>, [import("@aws/dynamodb-data-mapper").SyncOrAsyncIterable<StringToAnyObjectMap>, (import("@aws/dynamodb-data-mapper").BatchGetOptions | undefined)?]>;
export declare function mockBatchPut(items: unknown[]): jest.SpyInstance<AsyncIterableIterator<StringToAnyObjectMap>, [items: import("@aws/dynamodb-data-mapper").SyncOrAsyncIterable<StringToAnyObjectMap>]>;
export declare function mockBatchDelete(items: unknown[]): jest.SpyInstance<AsyncIterableIterator<StringToAnyObjectMap>, [items: import("@aws/dynamodb-data-mapper").SyncOrAsyncIterable<StringToAnyObjectMap>]>;
...
export declare function mockBadgerSdk({ addr, network, currBlock, }?: {
    addr?: string;
    network?: Network;
    currBlock?: number;
}): Promise<BadgerSDK>;
