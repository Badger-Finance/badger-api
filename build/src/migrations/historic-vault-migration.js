...
async function run() {
    ...
    const migrationData = await (0, migration_utils_1.getMigrationData)(migration_constants_1.HISTORIC_VAULT_TO_CHART_DATA_MIGRATION_ID);
    if (migrationData && migrationData.status === migration_enums_1.MigrationStatus.Complete) {
        console.log(`Nothing left to process, ${constants_1.SETT_HISTORIC_DATA} table copied to ${constants_1.CHART_DATA}. Exiting`);
        return;
    }
    ...
    for (const chain of chain_1.SUPPORTED_CHAINS) {
        ...
        await Promise.all(vaults.map(async (vault) => {
            ...
            for await (const item of mapper.query(historic_vault_snapshot_old_model_1.HistoricVaultSnapshotOldModel, { address: vault.address, timestamp: (0, dynamodb_expressions_1.greaterThan)(lastInsertedTimestamp) }, { scanIndexForward: true, limit: 200 })) {
                ...
                migratedSnapshots += 1;
                const migratedCnt = await (0, migration_utils_1.pushHistoricSnapshots)(historic_vault_snapshot_model_1.HistoricVaultSnapshotModel.NAMESPACE, historicSnapshot);
                lastItem = item;
            ...
            if (!migrationSequence) {
                ...
                migrationSequence.status = migratedSnapshots === 0 ? migration_enums_1.MigrationStatus.Complete : migration_enums_1.MigrationStatus.Process;
    ...
    const isEverySequenceMigrated = migrationSequences.every((seq) => seq.status === migration_enums_1.MigrationStatus.Complete);
    const migrationStatus = isEverySequenceMigrated ? migration_enums_1.MigrationStatus.Complete : migration_enums_1.MigrationStatus.Process;
    const migrationStatusText = migrationStatus === migration_enums_1.MigrationStatus.Complete ? 'Complete' : 'Process';
    await mapper.put(Object.assign(new migration_process_model_1.MigrationProcessData(), {
