...
exports.refreshAccountVaultBalances = exports.getLatestMetadata = exports.getClaimableBalanceSnapshot = exports.getCachedAccount = exports.getCachedBoost = exports.toVaultBalance = exports.queryCachedAccount = exports.getAccounts = exports.getBoostFile = void 0;
...
async function queryCachedAccount(address) {
    ...
    try {
        ...
        for await (const item of mapper.query(cached_account_model_1.CachedAccount, { address: checksummedAccount }, { limit: 1, scanIndexForward: false })) {
            return item;
        }
...
async function getCachedBoost(chain, address) {
    const mapper = (0, dynamodb_utils_1.getDataMapper)();
    for await (const entry of mapper.query(cached_boost_model_1.CachedBoost, { leaderboard: (0, dynamodb_utils_1.getLeaderboardKey)(chain), address: ethers_1.ethers.utils.getAddress(address) }, { limit: 1, indexName: 'IndexLeaderBoardRankOnAddressAndLeaderboard' })) {
        return entry;
    }
    return {
        address,
...
async function getCachedAccount(chain, address) {
    ...
    if (!cachedAccount.updatedAt || cachedAccount.updatedAt + sdk_1.ONE_MINUTE_MS < Date.now()) {
...
async function getClaimableBalanceSnapshot(chain, address, startBlock) {
    const mapper = (0, dynamodb_utils_1.getDataMapper)();
    for await (const entry of mapper.query(user_claim_snapshot_model_1.UserClaimSnapshot, { chainStartBlock: (0, dynamodb_utils_1.getChainStartBlockKey)(chain, startBlock), address: ethers_1.ethers.utils.getAddress(address) }, { limit: 1, indexName: 'IndexUnclaimedSnapshotsOnAddressAndChainStartBlock' })) {
        return entry;
    }
    return {
...
async function getLatestMetadata(chain) {
    const mapper = (0, dynamodb_utils_1.getDataMapper)();
    let result = null;
    for await (const metric of mapper.query(user_claim_metadata_1.UserClaimMetadata, { chain: chain.network }, { indexName: 'IndexMetadataChainAndStartBlock', scanIndexForward: false, limit: 1 })) {
        result = metric;
    }
...
async function refreshAccountVaultBalances(chain, account) {
    try {
        ...
        if (user) {
            ...
            if (userBalances) {
                const balances = await Promise.all(userBalances.filter(async (balance) => {
                    try {
