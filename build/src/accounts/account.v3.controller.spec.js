...
describe('AccountsController', () => {
    ...
    beforeEach(async () => {
        await (0, tests_utils_1.mockBadgerSdk)();
    ...
    describe('GET /v3/account', () => {
        describe('with no specified account', () => {
            it('returns a not found response', async (done) => {
                const { body } = await request.get('/v3/account').expect(network_status_enum_1.NetworkStatus.BadRequest);
        ...
        describe('with an invalid account input', () => {
            it('returns a bad request response', async (done) => {
                const { body } = await request.get('/v3/account?address=0xjintao').expect(network_status_enum_1.NetworkStatus.BadRequest);
                expect(body).toMatchSnapshot();
                done();
            });
        });
        describe('with a non participant account', () => {
            it('returns a default account response', async (done) => {
                ...
                const { body } = await request.get(`/v3/account?address=${constants_1.TEST_ADDR}`).expect(200);
                expect(body).toMatchSnapshot();
                done();
            });
        });
...
//# sourceMappingURL=account.v3.controller.spec.js.map