...
exports.getProtocolValueSources = exports.getVaultValueSources = exports.getRewardEmission = exports.getClaimableRewards = exports.getTreeDistribution = exports.DIGG_SHARE_PER_FRAGMENT = void 0;
...
async function getRewardEmission(chain, vault) {
    ...
    for (const schedule of activeSchedules) {
        ...
        if (cachedVault.boost.enabled && token.address === chain.getBadgerTokenAddress()) {
            ...
            const boostYieldSource = (0, yields_utils_1.createYieldSource)(vault, source_type_enum_1.SourceType.Emission, boostedName, boostedApr, boostRange);
            emissionSources.push(boostYieldSource);
        }
        const proRataYieldSource = (0, yields_utils_1.createYieldSource)(vault, source_type_enum_1.SourceType.Emission, token.name, proRataApr);
...
async function getVaultValueSources(chain, vaultDefinition) {
    ...
    const NO_COMPOUND_VAULTS = new Set([tokens_config_1.TOKENS.BREMBADGER, tokens_config_1.TOKENS.BVECVX, tokens_config_1.TOKENS.BCVX]);
    let sources = [];
    try {
        ...
        if (hasNoUnderlying) {
            sources = sources.filter((s) => s.type !== source_type_enum_1.SourceType.Compound && s.type !== source_type_enum_1.SourceType.PreCompound);
        }
        return sources;
    }
    catch (err) {
