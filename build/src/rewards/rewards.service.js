...
let RewardsService = class RewardsService {
  ...
  async getBouncerProof(chain, address) {
    ...
    const claim =
      fileContents.claims[address.toLowerCase()] || fileContents.claims[ethers_1.ethers.utils.getAddress(address)];
    if (!claim) {
  ...
  async rewardSchedulesVaultsList(chain, active) {
    ...
    return vaultsSchedules.reduce((acc, vaultSchedules) => {
      if (
        !vaultSchedules ||
        (vaultSchedules === null || vaultSchedules === void 0 ? void 0 : vaultSchedules.length) === 0
      )
        return acc;
...
RewardsService = tslib_1.__decorate([(0, common_1.Service)()], RewardsService);
