...
const loadSchedules_json_1 = tslib_1.__importDefault(
  require("@badger-dao/sdk-mocks/generated/ethereum/rewards/loadSchedules.json")
);
  ...
  describe("GET /v2/reward/schedules/<beneficiary>", () => {
    ...
    describe("with active param true", () => {
      it("returns schedules for default chain and one vault", async (done) => {
        ...
        const { body } = await request
          .get(`/v2/reward/schedules/${tokens_config_1.TOKENS.BBADGER}?active=true`)
          .expect(200);
        expect(body).toMatchSnapshot();
        done();
      });
    ...
    describe("with invalid param specified", () => {
      it("returns a 400, NotFound", async (done) => {
        ...
        const { body } = await request
          .get(`/v2/reward/schedules/unknowsvaultdata`)
          .expect(exceptions_1.NotFound.STATUS);
        expect(body).toMatchSnapshot();
        done();
      });
    });
