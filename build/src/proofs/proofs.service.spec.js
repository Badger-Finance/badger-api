...
describe("proofs.service", () => {
  ...
  describe("getBouncerProof", () => {
    it("throws a 404 when a chain is missing a bouncer file", async () => {
      ...
      await expect(service.getBouncerProof(tests_utils_1.TEST_CHAIN, constants_1.TEST_ADDR)).rejects.toThrow(
        `${tests_utils_1.TEST_CHAIN.network} does not have requested data`
      );
    });
    it("throws a 404 when a chain is missing an entry for the user in the bouncer file", async () => {
      ...
      await expect(service.getBouncerProof(tests_utils_1.TEST_CHAIN, tokens_config_1.TOKENS.BADGER)).rejects.toThrow(
        `No data for specified address: ${tokens_config_1.TOKENS.BADGER}`
      );
    });
