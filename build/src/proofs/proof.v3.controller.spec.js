...
describe('ProofsController', () => {
    ...
    describe('GET /v3/proofs', () => {
        it('returns 404 for a chain with no bouncer file', async (done) => {
            ...
            const { body } = await request.get(`/v3/proof?address=${constants_1.TEST_ADDR}`).expect(network_status_enum_1.NetworkStatus.NotFound);
        ...
        it('returns 404 for users not on the bouncer list', async (done) => {
            ...
            const { body } = await request.get(`/v3/proof?address=${badAddress}`).expect(network_status_enum_1.NetworkStatus.NotFound);
            expect(body).toMatchSnapshot();
            done();
        });
        it('returns 200 and the merkle proof for a user on the bouncer list', async (done) => {
            ...
            const { body } = await request.get(`/v3/proof?address=${constants_1.TEST_ADDR}`).expect(network_status_enum_1.NetworkStatus.Success);
            expect(body).toMatchSnapshot();
            done();
        });
    });
