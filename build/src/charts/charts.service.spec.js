...
describe('charts.service', () => {
    ...
    describe('loadVaultChartData', () => {
        it('returns requested vault chart data if exists', async () => {
            ...
            jest.spyOn(chartsUtils, 'queryVaultCharts').mockImplementation(async (_k) => constants_1.MOCK_VAULT_SNAPSHOTS.slice(0, 4).map((snapshot) => {
            ...
            const charts = await service.loadVaultChartData(constants_1.MOCK_VAULT_SNAPSHOT.address, sdk_1.ChartTimeFrame.Max, tests_utils_1.TEST_CHAIN);
