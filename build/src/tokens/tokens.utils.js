...
exports.mockBalance = exports.lookUpAddrByTokenName = exports.mergeTokensFullData = exports.cacheTokensInfo = exports.getCachedTokesInfo = exports.getFullTokens = exports.getFullToken = exports.getVaultTokens = exports.toBalance = void 0;
...
async function getVaultTokens(chain, vault, currency) {
    ...
    for await (const record of mapper.query(vault_token_balance_model_1.VaultTokenBalance, { vault: vault.vaultToken }, { limit: 1 })) {
        tokens = await Promise.all(record.tokenBalances.map(async (b) => ({
            ...b,
...
async function getCachedTokesInfo(tokensAddr) {
    const mapper = (0, dynamodb_utils_1.getDataMapper)();
    const tokensToGet = tokensAddr.map((addr) => Object.assign(new token_information_snapshot_model_1.TokenInformationSnapshot(), { address: ethers_1.ethers.utils.getAddress(addr) }));
    const tokensInfo = [];
    try {
...
async function cacheTokensInfo(tokens) {
    const mapper = (0, dynamodb_utils_1.getDataMapper)();
    const tokensInfoMeta = tokens.map((token) => Object.assign(new token_information_snapshot_model_1.TokenInformationSnapshot(), token));
    try {
...
function lookUpAddrByTokenName(chain, name) {
    ...
    return (_a = Object.values(tokensWithAddr).find((token) => token.lookupName === name)) === null || _a === void 0 ? void 0 : _a.address;
}
