  ...
  describe("indexVaultsHarvestsCompund", () => {
    it("should save all harvests from all chains to ddb", async () => {
      ...
      expect(put.mock.calls.length).toBe(
        chain_1.SUPPORTED_CHAINS.length *
          vault_harvests_on_chain_1.vaultHarvestsOnChainMock[constants_1.MOCK_VAULT_DEFINITION.address].length
      );
    });
    it("should get and save harvests only after the block of the last in ddb", async () => {
      const blocks = vault_harvests_on_chain_1.vaultHarvestsOnChainMock[constants_1.MOCK_VAULT_DEFINITION.address]
        .map((h) => h.block)
        .sort();
      const cutoffBlock = blocks[1];
      const conformingHarvests = vault_harvests_on_chain_1.vaultHarvestsOnChainMock[
        constants_1.MOCK_VAULT_DEFINITION.address
      ].filter((h) => h.block > cutoffBlock);
      jest.spyOn(vaultsUtils, "getLastCompoundHarvest").mockImplementation(async () => ({ block: cutoffBlock }));
      await (0, harvest_compound_indexer_1.indexVaultsHarvestsCompund)();
