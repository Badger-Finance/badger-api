...
async function indexPrices() {
    for (const chain of chain_1.SUPPORTED_CHAINS) {
        try {
            ...
            const contractTokenAddresses = chainTokens.filter((t) => t.type === pricing_type_enum_1.PricingType.Contract).map((t) => t.address);
            const lookupNames = chainTokens
                .filter((t) => t.type === pricing_type_enum_1.PricingType.LookupName)
                .map((t) => t.lookupName)
                .filter((name) => !!name);
            const onChainTokens = chainTokens.filter((t) => t.type !== pricing_type_enum_1.PricingType.Contract && t.type !== pricing_type_enum_1.PricingType.LookupName);
            ...
            const onChainPrices = await Promise.all(onChainTokens.map(async (t) => {
                try {
            ...
            await Promise.all(Object.values(priceUpdates).map(async (p) => {
                try {
...
function evaluateCoingeckoResponse(chain, result) {
    return Object.fromEntries(Object.entries(result).map((entry) => {
