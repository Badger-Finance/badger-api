...
function setupTestVault() {
  ...
  jest.spyOn(tokensUtils, "getFullToken").mockImplementation(async (_, tokenAddr) => {
    return (
      full_token_mock_1.fullTokenMockMap[tokenAddr] || full_token_mock_1.fullTokenMockMap[tokens_config_1.TOKENS.BADGER]
    );
  });
  ...
  jest.spyOn(vaultsUtils, "queryYieldEstimate").mockImplementation(async (vaultDefinition) => ({
    ...
    harvestTokens: [
      (0, tokens_utils_1.mockBalance)(full_token_mock_1.fullTokenMockMap[tokens_config_1.TOKENS.CVX], 10)
    ],
  ...
  jest.spyOn(vaultsUtils, "getCachedVault").mockImplementation(async (chain, vaultDefinition) => {
  ...
  jest.spyOn(vaultsUtils, "queryYieldSources").mockImplementation(async (vault) => {
    const performance = parseInt(vault.address.slice(0, 5), 16) / 100;
    const underlying = (0, yields_utils_1.createYieldSource)(
      vault,
      source_type_enum_1.SourceType.Compound,
      vaultsUtils.VAULT_SOURCE,
      performance
    );
    const badger = (0, yields_utils_1.createYieldSource)(
      vault,
      source_type_enum_1.SourceType.Emission,
      "Badger Rewards",
      performance
    );
    const fees = (0, yields_utils_1.createYieldSource)(
      vault,
  ...
  jest.spyOn(tokensUtils, "getVaultTokens").mockImplementation(async (_chain, vault, _currency) => {
    const token =
      full_token_mock_1.fullTokenMockMap[vault.underlyingToken] ||
    ...
    describe("error cases", () => {
      it("returns a 400 for invalide chain", async (done) => {
        const { body } = await request
          .get(`/v2/vaults/harvests/${TEST_VAULT}?chain=invalid`)
