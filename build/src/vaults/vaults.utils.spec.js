...
describe("vaults.utils", () => {
  beforeEach(() => {
    ...
    jest.spyOn(rewardsUtils, "getProtocolValueSources").mockImplementation(async (_chain, vault) => {
      return [
        (0, yields_utils_1.createYieldSource)(vault, source_type_enum_1.SourceType.TradeFee, "Test LP Fees", 1.13)
      ];
    });
  });
  function setupSdk() {
          ...
          harvests: [
            {
              timestamp,
              block,
              token: tokens_config_1.TOKENS.CRV_IBBTC,
              amount: ethers_1.BigNumber.from((int + 1 * 1.88e18).toString())
            }
          ],
          treeDistributions: [
            {
              timestamp,
              block,
              token: tokens_config_1.TOKENS.BCVXCRV,
              amount: ethers_1.BigNumber.from((int + 1 * 5.77e12).toString())
            },
            {
              timestamp,
              block,
              token: tokens_config_1.TOKENS.BVECVX,
              amount: ethers_1.BigNumber.from((int + 1 * 4.42e12).toString())
            }
          ]
        };
      });
  ...
  beforeEach(() => {
    ...
    jest.spyOn(rewardsUtils, "getRewardEmission").mockImplementation(async (_chain, vault) => {
      return [
        (0, yields_utils_1.createYieldSource)(vault, source_type_enum_1.SourceType.Emission, "Badger Rewards", 6.969, {
          min: 1,
          max: 2
        })
      ];
  ...
  const setupTestVaultHarvests = () => {
    ...
    jest.spyOn(sdk_1.BadgerGraph.prototype, "loadSett").mockImplementation(async ({ id, block }) => {
  ...
  describe("defaultVault", () => {
    it("returns a sett default fields", async () => {
      ...
      const expected = {
        ...
        bouncer:
          (_a = constants_2.MOCK_VAULT_DEFINITION.bouncer) !== null && _a !== void 0
            ? _a
            : bouncer_type_enum_1.BouncerType.None,
        ...
        type:
          constants_2.MOCK_VAULT_DEFINITION.protocol === sdk_1.Protocol.Badger
            ? sdk_1.VaultType.Native
            : sdk_1.VaultType.Standard,
        behavior:
          (_b = constants_2.MOCK_VAULT_DEFINITION.behavior) !== null && _b !== void 0 ? _b : sdk_1.VaultBehavior.None,
        lastHarvest: 0,
        yieldProjection: {
          yieldApr: 0,
          yieldTokens: [],
      ...
      const actual = await (0, vaults_utils_1.defaultVault)(
        tests_utils_1.TEST_CHAIN,
        constants_2.MOCK_VAULT_DEFINITION
      );
  ...
  describe("getCachedVault", () => {
    describe("no cached vault exists", () => {
      it("returns the default sett", async () => {
        ...
        (0, tests_utils_1.setFullTokenDataMock)();
        const cached = await (0, vaults_utils_1.getCachedVault)(
          tests_utils_1.TEST_CHAIN,
          constants_2.MOCK_VAULT_DEFINITION
        );
        const defaultVaultInst = await (0, vaults_utils_1.defaultVault)(
          tests_utils_1.TEST_CHAIN,
          constants_2.MOCK_VAULT_DEFINITION
        );
    ...
    describe("a cached vault exists", () => {
      it("returns the vault", async () => {
        ...
        const expected = await (0, vaults_utils_1.defaultVault)(
          tests_utils_1.TEST_CHAIN,
          constants_2.MOCK_VAULT_DEFINITION
        );
  ...
  describe("getVaultTokenPrice", () => {
    describe("look up non vault token price", () => {
      it("throws a bad request error", async () => {
        (0, tests_utils_1.setFullTokenDataMock)();
        await expect(
          (0, vaults_utils_1.getVaultTokenPrice)(tests_utils_1.TEST_CHAIN, tokens_config_1.TOKENS.BADGER)
        ).rejects.toThrow(exceptions_1.BadRequest);
      });
    });
    describe("look up malformed token configuration", () => {
      it("throws an unprocessable entity error", async () => {
        (0, tests_utils_1.setFullTokenDataMock)();
        await expect(
          (0, vaults_utils_1.getVaultTokenPrice)(tests_utils_1.TEST_CHAIN, ethers_1.ethers.constants.AddressZero)
        ).rejects.toThrow(token_error_1.TokenNotFound);
      });
    });
    describe("look up valid, properly configured vault", () => {
      it("returns a valid token price for the vault base on price per full share", async () => {
        ...
        (0, tests_utils_1.setFullTokenDataMock)();
        const vaultPrice = await (0, vaults_utils_1.getVaultTokenPrice)(
          tests_utils_1.TEST_CHAIN,
          constants_2.MOCK_VAULT_DEFINITION.address
        );
  ...
  describe("getVaultPerformance", () => {
    describe("no rewards or harvests", () => {
      it("returns value sources from fallback methods", async () => {
        ...
        (0, tests_utils_1.setFullTokenDataMock)();
        const result = await (0, vaults_utils_1.getVaultPerformance)(
          tests_utils_1.TEST_CHAIN,
          constants_2.MOCK_VAULT_DEFINITION
        );
  ...
  describe("getVaultHarvestsOnChain", () => {
    ...
    it("returns empty harvests for unknown vault", async () => {
      ...
      await expect(
        (0, vaults_utils_1.getVaultHarvestsOnChain)(tests_utils_1.TEST_CHAIN, "0x000000000000")
