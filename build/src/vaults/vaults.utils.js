...
exports.getLastCompoundHarvest = exports.getVaultHarvestsOnChain = exports.queryYieldEstimate = exports.queryYieldSources = exports.estimateVaultPerformance = exports.estimateHarvestEventApr = exports.loadVaultGraphPerformances = exports.estimateDerivativeEmission = exports.loadVaultEventPerformances = exports.getVaultPerformance = exports.getVaultTokenPrice = exports.getBoostWeight = exports.getStrategyInfo = exports.getCachedVault = exports.defaultVault = exports.VAULT_SOURCE = void 0;
...
async function defaultVault(chain, vault) {
    ...
    const [assetToken, vaultToken] = await Promise.all([(0, tokens_utils_1.getFullToken)(chain, depositToken), (0, tokens_utils_1.getFullToken)(chain, address)]);
...
async function getCachedVault(chain, vaultDefinition, currency = sdk_1.Currency.USD) {
    ...
    try {
        ...
        for await (const item of mapper.query(current_vault_snapshot_model_1.CurrentVaultSnapshotModel, { address: vaultDefinition.address }, { limit: 1, scanIndexForward: false })) {
...
async function getVaultPerformance(chain, vault) {
    ...
    const aggregatedSources = (0, yields_utils_1.aggregateSources)([...vaultSources, ...rewardEmissions, ...protocol], (s) => s.id);
...
async function loadVaultEventPerformances(chain, vault) {
    var _a;
    const incompatibleNetworks = new Set([sdk_1.Network.BinanceSmartChain, sdk_1.Network.Polygon, sdk_1.Network.Arbitrum]);
 ...
 */
function estimateDerivativeEmission(compoundApr, emissionApr, emissionCompoundApr, compoundingStep = 1, emissionStep = 1) {
...
function constructGraphVaultData(vault, settHarvests, badgerTreeDistributions) {
    ...
    const treeDistributionsByTimestamp = (0, sdk_1.keyBy)(badgerTreeDistributions, (distribution) => distribution.timestamp);
...
async function estimateVaultPerformance(chain, vault, data) {
    ...
    if (vault.address === tokens_config_1.TOKENS.BVECVX) {
        ...
        for (let i = 0; i < allHarvests.length - 1; i++) {
            ...
            if (sett) {
                const balance = (_b = (_a = sett.strategy) === null || _a === void 0 ? void 0 : _a.balance) !== null && _b !== void 0 ? _b : sett.balance;
    ...
    const compoundYieldSource = (0, yields_utils_1.createYieldSource)(vault, source_type_enum_1.SourceType.PreCompound, exports.VAULT_SOURCE, compoundApr * 100);
    valueSources.push(compoundYieldSource);
    // create the apy source for harvests
    const compoundApy = (1 + compoundApr / periods) ** periods - 1;
    const compoundedYieldSource = (0, yields_utils_1.createYieldSource)(vault, source_type_enum_1.SourceType.Compound, exports.VAULT_SOURCE, compoundApy * 100);
    ...
    for (const [token, amount] of tokensEmitted.entries()) {
        ...
        const emissionYieldSource = (0, yields_utils_1.createYieldSource)(vault, source_type_enum_1.SourceType.Distribution, tokenEmitted.name, emissionApr * 100);
        ...
        try {
            ...
            const compoundingSource = vaultValueSources.find((source) => source.type === source_type_enum_1.SourceType.PreCompound);
    ...
    if (flywheelCompounding > 0) {
        ...
        const flywheelYieldSource = (0, yields_utils_1.createYieldSource)(vault, source_type_enum_1.SourceType.Flywheel, sourceName, flywheelCompounding);
...
async function queryYieldSources(vault) {
    const valueSources = [];
    const mapper = (0, dynamodb_utils_1.getDataMapper)();
    for await (const source of mapper.query(yield_source_model_1.YieldSource, { chainAddress: vault.id }, { indexName: 'IndexApySnapshotsOnAddress' })) {
...
async function queryYieldEstimate(vault) {
    ...
    try {
        const mapper = (0, dynamodb_utils_1.getDataMapper)();
        for await (const item of mapper.query(yield_estimate_model_1.YieldEstimate, { vault: vault.address }, { limit: 1 })) {
            return item;
        }
...
async function getVaultHarvestsOnChain(chain, address, startFromBlock = null) {
    ...
    try {
        ...
        if (startFromBlock)
            listHarvestsArgs.startBlock = startFromBlock;
        sdkVaultHarvestsResp = await sdk.vaults.listHarvests(listHarvestsArgs);
    }
    catch (e) {
        console.warn(`Failed to get harvests list ${e}`);
    }
    if (!sdkVaultHarvestsResp || ((_a = sdkVaultHarvestsResp === null || sdkVaultHarvestsResp === void 0 ? void 0 : sdkVaultHarvestsResp.data) === null || _a === void 0 ? void 0 : _a.length) === 0) {
    ...
    const _extend_harvests_data = async (harvestsList, eventType) => {
        ...
        for (let i = 0; i < harvestsList.length; i++) {
            ...
            const harvestAmount = (0, sdk_1.formatBalance)(harvest.amount || ethers_1.BigNumber.from(0), depositTokenInfo.decimals);
            ...
            if (vaultGraph === null || vaultGraph === void 0 ? void 0 : vaultGraph.sett) {
                const balance = ethers_1.BigNumber.from(((_b = (_a = vaultGraph.sett) === null || _a === void 0 ? void 0 : _a.strategy) === null || _b === void 0 ? void 0 : _b.balance) || vaultGraph.sett.balance || 0);
                ...
                if (endOfCurrentHarvest) {
                    extendedHarvest.estimatedApr = await estimateHarvestEventApr(chain, harvestToken, startOfHarvest, endOfCurrentHarvest, harvest.amount, balance);
                }
            }
            vaultHarvests.push(extendedHarvest);
        }
...
async function getLastCompoundHarvest(vault) {
    const mapper = (0, dynamodb_utils_1.getDataMapper)();
    const query = mapper.query(harvest_compound_model_1.HarvestCompoundData, { vault }, { limit: 1, scanIndexForward: false });
