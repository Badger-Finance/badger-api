...
let VaultsService = VaultsService_1 = class VaultsService {
    async getProtocolSummary(chain, currency) {
        ...
        const summaries = await Promise.all(vaults.map(async (vault) => {
    ...
    async listVaultHarvests(chain) {
        const vaults = await chain.vaults.all();
        const harvestsWithSnapshots = await Promise.all(vaults.map(async (vault) => {
            return {
                vault: vault.address,
    ...
    async getVaultHarvests(chain, address) {
        ...
        const queryHarvests = mapper.query(harvest_compound_model_1.HarvestCompoundData, { vault: vault.address }, { indexName: 'IndexHarvestCompoundDataVault' });
        try {
...
VaultsService = VaultsService_1 = tslib_1.__decorate([
    (0, common_1.Service)()
], VaultsService);
