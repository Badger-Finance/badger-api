...
async function getVaultsDefinitionSeedData() {
    ...
    for (const chain of chain_1.SUPPORTED_CHAINS) {
        ...
        await Promise.all(registryVaults.map(async (vault) => {
            let compoundVault;
            try {
                compoundVault = await (0, indexer_utils_1.constructVaultDefinition)(chain, vault);
                if (compoundVault)
                    seedVaults.push(compoundVault);
            }
            catch (err) {
...
function saveSeedJSONFile(data, filename) {
    (0, fs_1.writeFileSync)((0, path_1.resolve)(__dirname, '../../../', dev_constants_1.VAULT_SEED_DIR, filename), JSON.stringify(data, null, 2));
}
