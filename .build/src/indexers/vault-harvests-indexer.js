...
exports.refreshVaultHarvests = void 0;
...
const vault_pending_harvest_model_1 = require("../aws/models/vault-pending-harvest.model");
...
async function refreshVaultHarvests() {
    await Promise.all(chain_1.SUPPORTED_CHAINS.map(async (chain) => {
        ...
        for (const vault of vaults) {
            ...
            const existingHarvest = await (0, vaults_utils_1.queryPendingHarvest)(vault);
            const harvestData = {
                vault: vault.address,
                yieldTokens: [],
                harvestTokens: [],
            ...
            if (vault.version && vault.version === sdk_1.VaultVersion.v1_5) {
                try {
                    ...
                    harvestData.harvestTokens = await Promise.all(pendingHarvest.tokenRewards.map(async (t) => (0, tokens_utils_1.toBalance)(await (0, tokens_utils_1.getFullToken)(chain, t.address), t.balance)));
                    ...
                    if (now - sdk_1.ONE_DAY_MS / 3 > harvestData.lastReportedAt) {
                        (0, discord_utils_1.sendPlainTextToDiscord)(`${chain.name} ${vault.name} (${vault.protocol}, ${vault.version}, ${(_d = vault.state) !== null && _d !== void 0 ? _d : sdk_1.VaultState.Open}) failed to harvest!`);
                        harvestData.lastReportedAt = now;
                    }
                }
                const pendingYield = await sdk.vaults.getPendingYield(vault.address);
                harvestData.yieldTokens = await Promise.all(pendingYield.tokenRewards.map(async (t) => (0, tokens_utils_1.toBalance)(await (0, tokens_utils_1.getFullToken)(chain, t.address), t.balance)));
            }
            else {
                shouldCheckGraph = true;
            }
            if (shouldCheckGraph) {
                ...
                if (badgerTreeDistributions.length > 0 &&
            ...
            const { strategy: { performanceFee }, } = cachedVault;
            ...
            try {
                await mapper.put(Object.assign(new vault_pending_harvest_model_1.VaultPendingHarvestData(), harvestData));
            }
            catch (err) {
...
exports.refreshVaultHarvests = refreshVaultHarvests;
//# sourceMappingURL=vault-harvests-indexer.js.map