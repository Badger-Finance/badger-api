...
let RewardsV3Controller = class RewardsV3Controller {
  async getBouncerProof(address, chain) {
    ...
    return this.rewardsService.getBouncerProof(
      chain_config_1.Chain.getChain(chain),
  ...
  async userClaimedSnapshotToDebankUser(chain, snapshot) {
    ...
    for (const record of snapshot.claimableBalances) {
      ...
      if (token.address === tokens_config_1.TOKENS.DIGG) {
        rewards[address] = (0, sdk_1.formatBalance)(
          ethers_1.BigNumber.from(balance).div(rewards_utils_1.DIGG_SHARE_PER_FRAGMENT),
          token.decimals
        );
      } else {
...
RewardsV3Controller = tslib_1.__decorate([(0, common_1.Controller)("/rewards")], RewardsV3Controller);
